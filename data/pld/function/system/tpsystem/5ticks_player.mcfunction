execute if score @s using_tpstone matches 1.. if score @s tp_cool_tick matches -1 if score @s tp_tick matches -1 if score @s feather_mainland matches 1 as @s[nbt={SelectedItem:{components:{custom_data:{id:"panling:tp_stone"}}}}] run function pld:system/tpsystem/using_stone_check
...
#大陆外
execute if score @s feather_mainland matches -1 if score @s tp_cool_tick matches -1 if score @s using_tpstone matches 1.. run tellraw @s[nbt={SelectedItem:{components:{custom_data:{id:"panling:tp_stone"}}}}] {"translate":"pl.info.tp_mirror_resis3"}
