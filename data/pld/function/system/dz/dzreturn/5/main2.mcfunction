...
#将条目合并入裝備&羽毛
execute unless block ~3 ~-1 ~ minecraft:dispenser{Items:[{Slot:5b,components:{"minecraft:custom_data":{named:1}}}]} unless block ~3 ~-1 ~ minecraft:dispenser{Items:[{Slot:5b,id:"minecraft:feather"}]} as @s run data modify block ~3 ~-1 ~ Items[{Slot:5b}].tag.display.Lore insert -2 from block ~ ~2 ~ front_text.messages[0]
