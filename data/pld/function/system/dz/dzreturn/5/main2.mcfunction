...
#生成告示牌合并内容
execute as @s run setblock ~ ~2 ~ minecraft:oak_sign[custom_data={front_text:{messages:['[{"translate":"pl.lore.maker"},{"nbt":"Items[{id:\'minecraft:player_head\'}].tag.SkullOwner.Name","block":"~ ~-1 ~","color":"yellow","italic":false}]','{"translate":""}','{"translate":""}','{"translate":""}']}}] replace
...
execute unless block ~3 ~-1 ~ minecraft:dispenser[custom_data={Items:[{Slot:5b,components:{"minecraft:custom_data":{named:1}}}]}] if block ~3 ~-1 ~ minecraft:dispenser[custom_data={Items:[{Slot:5b,id:"minecraft:feather"}]}] as @s run data modify block ~3 ~-1 ~ Items[{Slot:5b}].tag.display.Lore insert -3 from block ~ ~2 ~ front_text.messages[0]
